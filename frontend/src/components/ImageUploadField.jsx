 ...
 *  - text input for pasting a URL (still supported for external images)
 *  - "Upload" button to pick a file from the user's device (any size, image/*)
 *
 * Props:
...
export default function ImageUploadField({
  ...
  const handleFile = async (e) => {
    ...
    if (!file.type.startsWith('image/')) {
      toast({ title: 'Not an image file', description: file.type || 'Please pick an image.' });
      e.target.value = '';
      return;
    }
    setUploading(true);
